...
st.set_page_config(page_title="AI Trading Advisor", page_icon="📈", layout="wide")

st.title("🌟 AI Trading Advisor V2.0")
...
def get_market_signal(ticker):
    try:
        ...
        if not news_data:
            return "No news found.", "🟡 HOLD"

        headline = news_data[0].get('headline', "No Headline")
        
        # 2. Get Price Data from yfinance
        stock = yf.Ticker(ticker)
        hist = stock.history(period="1d", interval="1m")
        if hist.empty:
            return headline, "⚪️ MARKET CLOSED"
        
        price = hist['Close'].iloc[-1]
        change = ((price - hist['Open'].iloc[0]) / hist['Open'].iloc[0]) * 100
        
        # 3. Decision Logic
        bad_news = ["war", "plunge", "hit", "drop", "fire", "attack", "outage"]
        good_news = ["growth", "buy", "target", "soar", "deal", "ai"]
        
        headline_low = headline.lower()
        is_bad = any(w in headline_low for w in bad_news)
        is_good = any(w in headline_low for w in good_news)

        if is_bad and change < -0.5:
            advice = f"🔴 RECOMMEND: SELL (News: {change:.2f}%)"
        elif is_good and change > 0.5:
            advice = f"🟢 RECOMMEND: BUY (News: {change:.2f}%)"
        else:
            advice = f"🟡 RECOMMEND: HOLD"
...
if st.button('🔍 Start Real-Time Scan'):
    ...
    # We create a container to show the alerts as they happen
    ...
    for i, ticker in enumerate(WATCHLIST):
        ...
        headline, advice = get_market_signal(ticker)
